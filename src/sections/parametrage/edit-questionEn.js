...
import { updateQuestionEn } from 'src/firebase/firebaseServices';
...
export const EditQuestionEn = ({ handleClose, isOpen, data }) => {
    ...
    const formik = useFormik({
        ...
        onSubmit: async (values, helpers) => {
            if (answerLibelle.length != 0 && categLibelle.length != 0 && defiLibelle.length != 0) {
                ...
                updateQuestionEn(datas, data.id)
                    .then(() => {
                        helpers.resetForm();
                        handleClose();
                            ...
                            <CardContent>
                                ...
                                >
                                    ...
                                    <Stack direction={'row'} spacing={1}>
                                        <Typography variant='subtitle2'>{data.pilier.libelleEn}</Typography>
                                        <SvgIcon>
                                        ...
                                        >
                                            ...
                                            {piliers.map((pilier, index) => {
                                                return (<MenuItem value={JSON.stringify(pilier)}
                                                    key={index}>{pilier.libelleEn}</MenuItem>)
                                            })}
                                        ...
                                        >
                                            {formik.touched.pilier && formik.values.pilier != "" ?
                                                defis.filter(item => item.pilier?.id == JSON.parse(formik.values.pilier).id).map((defi, index) => (
                                                    <MenuItem key={index}
                                                        value={defi.libelleEn}>
                                                        <Checkbox checked={defiLibelle.indexOf(defi.libelleEn) > -1} />
                                                        <ListItemText primary={defi.libelleEn} />
                                                    </MenuItem>
                                                )) :
                                                defis.map((defi, index) => (
                                                    <MenuItem key={index}
                                    ...
                                    <FormControl variant="filled" fullWidth>
                                        ...
                                        >
                                            {answers.map((answer, index) => (
                                                <MenuItem key={index}
                                                    value={answer.libelleEn}>
                                                    <Checkbox checked={answerLibelle.indexOf(answer.libelleEn) > -1} />
                                                    <ListItemText primary={answer.libelleEn} />
                                                </MenuItem>
                                            ))}
                                        </Select>
